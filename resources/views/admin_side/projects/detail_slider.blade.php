...
@section('content')
                        ...
                        <div class="ps-3">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb mb-0 p-0 align-items-center">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">Detail Slider</li>
                                </ol>
                            </nav>
                        </div>
                        <div class="ms-auto">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Detail Slider</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="Add_detail_slider_Form" action="" method="POST" enctype="multipart/form-data">
                        <div class="row">
                            <div class="col-md-12">
    ...
    <div id="edit_detail_slider_modal" class="modal custom-modal fade" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <form id="Edit_detail_slider_Form" action="" method="POST" enctype="multipart/form-data">
                        <input type="hidden" name="detail_slider_id">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
    ...
    <script>
        $(document).ready(function() {
            ...
            function getDetailSlider() {

                $.ajax({

                    url: '{{ url('/get-detail-slider') }}',
                    type: 'get',
                    async: false,
                    dataType: 'json',
                    ...
                    success: function(data) {
                        ...
                        for (i = 0; i < data.length; i++) {
                            ...
                            html += '<tr>' +
                                '<td>' + c + '</td>' +
                                '<td>' + data[i].title + '</td>' +
                                '<td><img src="{{ asset('storage/app/public/uploads/detail-slider/') }}/' +
                                data[i].image +
                                '" width="80px" height="80px" ></td>' +
                                '<td>' + data[i].created_at + '</td>' +
                                '<td> <div class="d-flex align-items-center gap-3 fs-6">' +
                                '<a href="#" class="text-warning btn_edit_project" data="' + data[i]
                                .id +
            ...
            //Add Slider
            $('#Add_detail_slider_Form').on('submit', function(e) {
                e.preventDefault();

                let formData = new FormData($('#Add_detail_slider_Form')[0]);

                $.ajax({
                    type: "POST",
                    url: "{{ url('store-detial-slider') }}",
                    data: formData,
                    headers: {
                    ...
                    success: function(response) {
                        ...
                        if (response.status == 200) {
                            ...
                            $(".close").click();
                            $('#Add_detail_slider_Form').find('input').val("");
                            getDetailSlider();
            ...
            $('#projectTable').on('click', '.btn_edit_project', function(e) {
                ...
                $('#edit_detail_slider_modal').modal('show');

                $.ajax({

                    type: 'ajax',
                    method: 'get',
                    url: '{{ url('edit-detail-slider') }}',
                    data: {
                        id: id
                    },
                    async: false,
                    ...
                    success: function(data) {
                        ...
                        $('input[name=detail_slider_id]').val(data.id);
                        $('input[name=title]').val(data.title);

                        $('#store_image').html(
                            '<img src="{{ asset('storage/app/public/uploads/detail-slider/') }}/' +
            ...
            $('.update_projects').on('click', function(e) {
                ...
                let EditFormData = new FormData($('#Edit_detail_slider_Form')[0]);

                $.ajax({
                    type: "POST",
                    url: "{{ url('update-detail-slider') }}",
                    data: EditFormData,
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    ...
                    success: function(response) {
                        ...
                        if (response.status == 200) {
                            $('#edit_detail_slider_modal').modal('hide');
                            $('#Edit_detail_slider_Form').find('input').val("");
